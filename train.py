...
def cord(city):
    ...
    dir = {
        "bengaluru": (19.6625054, 78.4953182),
        "delhi": (18.6804717, 78.0606503),
        "lucknow": (18.4348833, 79.0981286),
        "delhi": (17.2484683, 80.006904),
    }
    return dir[city]
...
def weekly_update(city, model):
    ...
    new_value2 = end
    df = df.append(
        {"last updated date": new_value2, "retrain datetime": new_value1},
        ignore_index=True,
    )
...
def heatwave_train_model(city):
    ...
    one_prediction_file_name = "versioning/one/Heat wave/1_{}_prediction.csv".format(
        city
    )
    ...
    two_prediction_file_name = "versioning/two/Heat wave/2_{}_prediction.csv".format(
        city
    )
    two_data_file_name = "versioning/two/Heat wave/2_{}_data.csv".format(city)

    three_prediction_model_name = "versioning/three/Heat wave/3_{}_model.json".format(
        city
    )
    three_prediction_file_name = (
        "versioning/three/Heat wave/3_{}_prediction.csv".format(city)
    )
    three_data_file_name = "versioning/three/Heat wave/3_{}_data.csv".format(city)

    four_prediction_model_name = "versioning/four/Heat wave/4_{}_model.json".format(
        city
    )
    four_prediction_file_name = "versioning/four/Heat wave/4_{}_prediction.csv".format(
        city
    )
    ...
    winner_prediction_file_name = "winner/Heat wave/winner_{}_prediction.csv".format(
        city
    )
